...
const CONCESSIONARIA_LIST = [
  { code: "AMAZONAS_ENERGIA", name: "Amazonas Energia" },
  { code: "CELESC", name: "Celesc (SC)" },
  { code: "CEMIG", name: "Cemig (MG)" },
  { code: "COPEL", name: "Copel (PR)" },
  { code: "CPFL", name: "CPFL Energia" },
  { code: "EDP_ES", name: "EDP Espírito Santo" },
  { code: "EDP_SP", name: "EDP São Paulo" },
  { code: "ENEL", name: "Enel Brasil" },
  { code: "EQUATORIAL", name: "Equatorial Energia" },
  { code: "FECOERGS", name: "Fecoergs (RS)" },
  { code: "LIGHT", name: "Light (RJ)" },
  { code: "NEOENERGIA", name: "Neoenergia" },
  { code: "RORAIMA_ENERGIA", name: "Roraima Energia" },
];
...
export const RagConcessionarias = () => {
  ...
  useEffect(() => {
    const fetchRagFiles = async () => {
      try {
        setIsLoading(true);
        const data = await ragController.listRagDealership(
  ...
  const handleConfirmUpload = async () => {
    try {
      ...
      await ragController.uploadRagDealership({
        data,
        dealership: selectedConcessionaria,
      });

      toast({
        title: "Upload Concluído",
        description: `Arquivo enviado para a base da ${selectedConcessionaria}.`,
        variant: "sucess",
      });

      const updatedData = await ragController.listRagDealership(
        selectedConcessionaria,
      );
  ...
  const handleDelete = async (id: string) => {
    try {
      await ragController.deleteRagDealership({ id });
  ...
  const selectedConcessionariaName =
    CONCESSIONARIA_LIST.find((c) => c.code === selectedConcessionaria)?.name ||
    selectedConcessionaria;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Gerencie a base de conhecimento por concessionária
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            className="min-w-[240px] px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-green-500 bg-slate-50 dark:bg-slate-800 cursor-pointer transition-all hover:border-green-300"
          >
            {CONCESSIONARIA_LIST.map((c) => (
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <h3 className="text-xl font-bold text-slate-700 dark:text-white mb-2">
            {isUploading
              ? "Enviando arquivo..."
              : `Upload de PDF para ${selectedConcessionariaName}`}
          </h3>
          <p className="text-slate-500 dark:text-slate-400 text-sm max-w-md mx-auto">
            {isUploading
              ? "Aguarde enquanto processamos e indexamos seu documento na base vetorial."
              : "Arraste ou clique para selecionar. Os arquivos serão processados para alimentar a inteligência do RAG."}
          </p>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex flex-col lg:flex-row gap-4 items-center justify-between bg-slate-50/50 dark:bg-slate-800/30">
          ...
          <div className="flex items-center gap-4 w-full lg:w-auto justify-between lg:justify-end">
            ...
            <div className="px-4 py-2 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-xl border border-green-100 dark:border-green-800 text-xs font-bold">
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center p-20 space-y-4">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-medium">
                Sincronizando arquivos da {selectedConcessionariaName}...
              </p>
            </div>
          ) : (
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-800">
            <div className="bg-slate-900 dark:bg-slate-800 p-6 flex items-center justify-between text-white">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-bold text-lg">Confirmar Upload</h3>
                  <p className="text-xs text-slate-400">
                    Prepare o documento para indexação
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-5">
              <div className="space-y-3">
                ...
                <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-2xl border border-green-100 dark:border-green-800">
                  <p className="text-[10px] font-bold text-green-400 uppercase tracking-widest mb-1">
                    Destino da Indexação
                  </p>
                  <div className="flex items-center gap-3">
                    <Building2 className="w-4 h-4 text-green-600 dark:text-green-400" />
                    <p className="text-sm font-bold text-green-700 dark:text-green-300">
                      {selectedConcessionariaName} ({selectedConcessionaria})
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="Descreva brevemente o que este documento contém (ex: Normas técnicas, Contratos, etc)"
